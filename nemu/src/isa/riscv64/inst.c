...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori        , I, R(dest) = src1^SEXT(BITS(imm,11,0),12));
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori         , I, R(dest) = src1|SEXT(BITS(imm,11,0),12));
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", and         , I, R(dest) = src1&SEXT(BITS(imm,11,0),12));
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi_li_mv  , I, R(dest) = src1 + imm);
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti        , I, R(dest) = (sword_t)src1<(sword_t)SEXT(imm,12)?1:0);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw        , R, R(dest) = SEXT((int32_t)BITS(src1,31,0)>>BITS(src2,31,0),32));
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt         , R, R(dest) = (sword_t)src1<(sword_t)src2?1:0);
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu_snez   , R, R(dest) = src1<src2?1:0);
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw        , R, R(dest) = SEXT(BITS(src1,31,0)>>BITS(src2,31,0),32));
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw        , R, R(dest) = SEXT(BITS(src1,31,0)<<BITS(src2,31,0),32));
