...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw          , I, R(dest) = SEXT(Mr(src1 + imm, 4)&0xffff,32));
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti        , I, R(dest) = (int64_t)src1<(int64_t)SEXT(imm,12)?1:0);
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw          , S, Mw(src1 + imm, 4, src2&0xffff));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq_beqz    , B, if(src1==src2)s->dnpc=s->pc+imm);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne_bnez    , B, if(src1!=src2)s->dnpc=s->pc+imm);
